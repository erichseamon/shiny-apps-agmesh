...
layout(matrix(c(1,2,3,4),1, 1, byrow=TRUE))
#-by month
boxplot(claims ~ month, data = newmat5a, names =unique(newmat5a$month), ylab = "claim frequency", xlab = "Months", las = 2, main= paste("Washington Claim Frequency by Month, 2010-2015,", " for", input$commodity, sep=""))
...
output$commoditycontrols <- renderUI({
  ...
  selectizeInput("commodity", "STEP 1: Choose a commodity", uf3[,3], choices = as.vector(uf3[,3]), selected = "WHEAT")
