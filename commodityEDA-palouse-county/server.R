...
myfiles <- read.csv("lag_palouse1.csv", strip.white = TRUE)
myfiles <- data.frame(myfiles)
myfiles$prpet <- myfiles$pr - myfiles$pet

myfiles_allyears <- myfiles
#myfiles_allyears <- subset(myfiles, , c(pr, pdsi, pet, tmmx, prpet, loss, state, commodity, county, year))
...
#myfiles_allyears[8:15] <- scale(myfiles_allyears[8:15], center = TRUE, scale = TRUE)
...
myfiles_allyears <- subset(myfiles, , c(pr, pdsi, pet, tmmx, erc, soil_moisture_shorterm, soil_moisture_longterm, loss, count, countratio, county, commodity, damagecause, year))
...
myfiles_allyears <- subset(myfiles_allyears, state = input$state3)
...
myfiles_allyears[2:23] <- scale(myfiles_allyears[2:23], center = TRUE, scale = TRUE)
...
plot(test$cube_loss,pr.nn_,col='red',main='Real vs predicted NN',pch=18,cex=0.7)
abline(0,1,lwd=2)
legend('bottomright',legend='NN',pch=18,col='red', bty='n')

#plot(test[ncol(test)],pr.lm,col='blue',main='Real vs predicted lm',pch=18, cex=0.7)

plot(test$cube_loss,pr.lm,col='blue',main='Real vs predicted lm',pch=18, cex=0.7)
abline(0,1,lwd=2)
legend('bottomright',legend='LM',pch=18,col='blue', bty='n', cex=.95)
...
myfiles_allyears$loss_unscaled <- myfiles_allyears$loss
#myfiles_allyears$loss <- scale(myfiles_allyears$loss, center = TRUE, scale = FALSE)
myfiles_allyears[1:13] <- scale(myfiles_allyears[1:13], center = TRUE, scale = TRUE)
...
tree.1 <- rpart(form,data=myfiles_allyears, method="anova", control=rpart.control(minsplit=20,cp=0))
...
rpart.plot(tree.1, type = 4, extra = "auto", branch.lty=1, box.palette="GnBu", node.fun=node.fun, tweak = 1.2)

library(partykit)
rparty.fit <- as.party(tree.1)
...
#form <- as.formula(loss ~ tmmx + pr + pdsi + prpet + erc, data=myfiles_allyears)
tree.1 <- rpart(form,data=myfiles_allyears, method="anova", control=rpart.control(minsplit=30,cp=0))
...
rpart.plot(tree.1, type = 4, extra = "auto", branch.lty=3, box.palette="GnBu", node.fun=node.fun, tweak = 1.2)
...
output$plot8g2 <- renderDataTable({
  req(input$commodity)
  withProgress(message = 'Working', value = 0, {
...
myfiles <- read.csv("1989-2015_combined_revised.csv", strip.white = TRUE)
#myfiles_allyears <- subset(myfiles, , c(pr, pdsi, pet, tmmx, erc, countratio, loss, acres, count, county, year))
myfiles_allyears <- subset(myfiles, , c(tmmn, rmin, rmax, fm100, fm1000, pr, pdsi, pet, tmmx, prpet, erc, soil_moisture_shorterm, soil_moisture_longterm, countratio, loss, acres, count, county, year))
