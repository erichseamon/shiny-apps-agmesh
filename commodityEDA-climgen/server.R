...
#  input$myLoader #create a dependency on the button, per Shiny examples.
  ...
  output$climgen2 <- renderDataTable({
#req(input$commodity)
...
climatecalc <- function(statevar, statesclim, startdatec, enddatec) {
  ...
  statevar2 <- paste("get", statevar, sep="")
  gfunction <- eval(parse(text = statevar2)) 
  ...
  for (i in 1:monthspan) {
    ...
    assign(paste(statevar2, "final", sep=""), final)
    
  }
  
  return(get(paste(statevar2, "final", sep="")))

  
}
setwd("/dmine/data/tmp")
layout(matrix(c(1,2),1, 2, byrow=TRUE))


final <- climatecalc(input$vari2, input$state, input$time1, input$time2)
setwd("/dmine/data/tmp")
print("test")
write.table(final, file = paste(input$vari2, "_", input$time1, "_", input$time2, sep=""))
...
getPrecip <- function(gfunction, states, statevar, startDate, endDate){
...
urlz <- paste("http://thredds.northwestknowledge.net:8080/thredds/dodsC/agg_met_", statevar, "_1979_CurrentYear_CONUS.nc", sep="")
 
  fabric <- webdata(url = urlz, 
                    variables = statevar, 
  ...
  colnames(precipData_result_frame) <- c(statevar)
...
#statesTSColin <- c(input$state)
#startTSColin <- input$timerange[1]
#endTSColin <- input$timerange[2]
