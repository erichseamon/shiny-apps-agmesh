...
xx_damage <- readRDS(file = "/dmine/data/USDA/crop_indemnity_originals_aggregated/commodities_damagecause.rds")
 ...
 id <- "1RxLEi0DiwMZFIw5CSBaEFjlLG67bKyLV" # google file ID
  xx_damage2 <- fread(sprintf("https://docs.google.com/uc?id=%s&export=download", id), header = TRUE)
  xx_damage2 <- xx_damage2[,2:12]
  
  colnames(xx_damage2) <- c("year", "STATE_NAME", "NAME", "commodity", "damagecause", "loss", "count", "acres", "lossperacre", "lossperclaim", "acresperclaim")
  xx_damage2$State <- state.name[match(xx_damage2$State,state.abb)]

xx_damage2$STATE_NAME <- state.name[match(xx_damage2$STATE_NAME, state.abb)]
...
xxx2 <- subset(xx_damage2, year == input$year7 & commodity == input$commodityUS_damage & damagecause == input$damageUS_damage )
...
m_a <- merge(counties_conus, xxx2, by=c("STATE_NAME", "NAME"))
...
pal <- colorNumeric(rev(brewer.pal(9, "Spectral")), na.color = "#ffff00",
                    domain = eval(parse(text=paste("m$", input$loss7, sep=""))))
...
pal2 <- colorNumeric(brewer.pal(9, "GnBu"), na.color = "#ffff00",
                    domain = eval(parse(text=paste("m$", "loss", sep=""))))

pal3 <- colorNumeric(rev(brewer.pal(9, "Spectral")), na.color = "#ffff00",
                    domain = eval(parse(text=paste("m$", "log10loss", sep=""))))
...
m2$count <- as.numeric(m2$count)
if(input$loss7 == 'loss') {

leaflet(data = m2) %>% addProviderTiles("Stamen.TonerLite") %>% fitBounds(exte[1], exte[3], exte[2], exte[4]) %>% addPolygons(color = ~pal(loss), popup = markers$label,  weight = 1) %>% addLegend(pal = pal, values = ~loss, opacity = 0.5, title = NULL, position = "bottomright")
...
leaflet(data = m2) %>% addProviderTiles("Stamen.TonerLite") %>% fitBounds(exte[1], exte[3], exte[2], exte[4]) %>% addPolygons(color = ~pal3(log10loss), popup = markers$label,  weight = 1) %>% addLegend(pal = pal3, values = ~log10loss, opacity = 0.5, title = NULL, position = "bottomright")




#leaflet(data = m) %>% addProviderTiles("Stamen.TonerLite") %>% fitBounds(exte[1], exte[3], exte[2], exte[4]) %>% addPolygons(color = ~pal3(eval(parse(text=paste("m$", input$loss7, sep="")))), popup = markersa$labela,  weight = 0) %>% addPolygons(data = m2, color = ~pal3(eval(parse(text=paste("m2$", input$loss7, sep="")))), fillOpacity = 0, weight = 1, popup = markers$label) %>% addLegend(pal = pal3, values = ~eval(parse(text=paste("m$", "loss", sep=""))), opacity = 0.5, title = NULL, position = "bottomright") 

} else if (input$loss7 == 'count') { 

leaflet(data = m2) %>% addProviderTiles("Stamen.TonerLite") %>% fitBounds(exte[1], exte[3], exte[2], exte[4]) %>% addPolygons(color = ~pal(count), popup = markers$label,  weight = 1) %>% addLegend(pal = pal, values = ~count, opacity = 0.5, title = NULL, position = "bottomright")


#leaflet(data = m2) %>% addProviderTiles("Stamen.TonerLite") %>% fitBounds(exte[1], exte[3], exte[2], exte[4]) %>% addPolygons(color = ~pal(eval(parse(text=paste("m$", input$loss7, sep="")))), popup = markersa$labela, weight = 1) %>% addPolygons(data = m2, color = ~pal(eval(parse(text=paste("m2$", input$loss7, sep="")))), fillOpacity = 0, weight = 1, popup = markers$label) %>% addLegend(pal = pal, values = ~eval(parse(text=paste("m$", "count", sep=""))), opacity = 0.5, title = NULL,  position = "bottomright")

} else if (input$loss7 == 'lossperclaim') {

leaflet(data = m2) %>% addProviderTiles("Stamen.TonerLite") %>% fitBounds(exte[1], exte[3], exte[2], exte[4]) %>% addPolygons(color = ~pal(lossperclaim), popup = markers$label,  weight = 1) %>% addLegend(pal = pal, values = ~lossperclaim, opacity = 0.5, title = NULL, position = "bottomright")

#} else if (input$loss7 == 'lossperacre') {

#leaflet(data = m_a) %>% addProviderTiles("Stamen.TonerLite") %>% fitBounds(exte[1], exte[3], exte[2], exte[4]) %>% addPolygons(color = ~pal3(lossperacre), popup = markers$label,  weight = 1) %>% addLegend(pal = pal3, values = ~lossperacre, opacity = 0.5, title = NULL, position = "bottomright")
