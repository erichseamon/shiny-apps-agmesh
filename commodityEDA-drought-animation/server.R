...
#--change to lowercase DT2!!
  ...
  b <- barplot(DT7$LOSS, names.arg = DT7$DAMAGECAUSE, las=2, col = newmatrix, horiz=TRUE)
  #text(bb, midpoint_loss, labels=mz$loss, srt=90)
  #plot(m, col = newmatrix, main = paste(input$state, " crop loss $ \n", " ", plotyear, "\n", plotcommodity, sep=""))
...
inputcom <- gsub(" ", "", inputcom, fixed = TRUE)
  dir <- paste("https://dmine.io/waf/predictor_USDA/agmesh-scenarios/", input$state, "/month_png/timelapse/", inputcom, "_timelapse.mp4", sep="")
...
output$commoditycontrols <- renderUI({
   uniquez <<- list.files(paste("/dmine/data/USDA/agmesh-scenarios/", input$state, "/month_png/timelapse/", sep=""))
   elems <- unlist( strsplit( uniquez, "\\." ) )
   elems2 <- data.frame(unlist( strsplit( elems[], "\\_" ) ))
   colnames(elems2) <- c("commodity")
   commodity <- subset(elems2, commodity != "mp4" & commodity != "timelapse")
   colnames(commodity)<- c("commodity_name")
   uf2 <- matrix( elems2 , ncol = 4 , byrow = TRUE )
   uf3 <- as.data.frame( uf2 )

   #uf3 <- subset(uf2, V1 == input$year)
  selectizeInput("commodity", "Choose a commodity", commodity$commodity_name, choices = as.vector(commodity$commodity_name), selected = "WHEAT", multiple = FALSE)
